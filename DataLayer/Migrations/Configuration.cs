namespace EFDataLayer.Migrations
{
    ...
    internal sealed class Configuration : DbMigrationsConfiguration<EFDataLayer.ChurchContext>
    {
        ...
        protected override void Seed(EFDataLayer.ChurchContext context)
        {
            ...
            List<PrayerRequestUpdate> updates = new List<PrayerRequestUpdate>
            {
                PrayerRequestUpdate.Create(
                        prayerRequestId: context.PrayerRequests.Where(pr => pr.Title == "Fransico D'Anconia's cataract surgery.").First().Id,
                        updateInformation: "The surgery was successful. Pray for continued recovery."
                    ),
                PrayerRequestUpdate.Create(
                        prayerRequestId: context.PrayerRequests.Where(pr => pr.Title == "Fransico D'Anconia's cataract surgery.").First().Id,
                        updateInformation: "2 week check up an doc says Francisco's eyes look great!"
                    )
            };

            //prayerRequests.ForEach(pr => context.PrayerRequests.AddOrUpdate(pr));
            //context.SaveChanges();
            context.PrayerRequests.Where(pr => pr.Title == "Fransico D'Anconia's cataract surgery.").First().Updates = updates;
            context.SaveChanges();
        }
    }
}
